...
def main():
	...
	parser.add_argument('-u','--alturl', help='Outputs a custom URL followed by the channel number, Useful for running scripts', required=False, default=config["alturl"], action="store_true")
	...
	config["alturl"] = commandLineArgs["alturl"]
	...
	generatePlaylists(config["server"], config["RTMP"], config["service"], config["quality"], config["minQuality"], config["guideLookAheadMinutes"], commandLineArgs["find"].lower(), authSign, jsonGuide1, jsonGuide2, config["alturl"])
	generatePlaylistXML(config["server"], config["RTMP"], config["service"], config["quality"], authSign, jsonGuide1, jsonGuide2, config["tvheadend"], config["alturl"])
...
def generatePlaylists(server, rtmp, service, streamQuality, minQuality, guideLookAheadMinutes, find, authSign, jsonGuide1, jsonGuide2, alturl):
	...
	if (foundChannels == 0):
		...
		writePlaylistFile("SmoothStreamsTV.m3u8", m3u8)

def generatePlaylistXML(server, rtmp, service, streamQuality, authSign, jsonGuide1, jsonGuide2, tvheadend, alturl):
	'''build strings of channels in m3u8 and xspf formats'''
	...
	if alturl:
		...
		urlTemplate = 'http://{0}.smoothstreams.tv:9100/{1}/ch{2}q{3}.stream/playlist.m3u8?wmsAuthSign={4}=='
	if tvheadend:
		m3u8TrackTemplate = '#EXTINF:-1 tvg-name="{0}" tvg-id="{2}.smoothstreams.tv" tvh-chnum="{2}" tvg-logo="http://guide.smoothstreams.tv/assets/images/channels/{2}.png",{0}\n{1}\n'
	else:
	...
	xmlTemplate = ('<?xml version="1.0" encoding="UTF-8"?>\n' +
		'<tv generator-info-name="SmoothPlaylist.py -- version 1.1.1 Stankness" generator-info-url="https://bitbucket.org/stankness/">\n{0}{1}</tv>')
	...
	if (foundPrograms == 0):
		...
		writePlaylistFile("SmoothStreamsTV-xml.xml", xmlTemplate.format(xmlChannels, xmlPrograms))
